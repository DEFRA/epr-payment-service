...
IF NOT EXISTS (
    SELECT * FROM [__EFMigrationsHistory]
    WHERE [MigrationId] = N'20251120174915_extend-effectiveto-dates'
)
BEGIN
    IF EXISTS (SELECT * FROM [sys].[identity_columns] WHERE [name] IN (N'Id', N'Name') AND [object_id] = OBJECT_ID(N'[Lookup].[FeeTypes]'))
        SET IDENTITY_INSERT [Lookup].[FeeTypes] ON;
    EXEC(N'INSERT INTO [Lookup].[FeeTypes] ([Id], [Name])
    VALUES (9, N''Producer Resubmission Fee''),
    (10, N''Compliance Scheme Resubmission'')');
    IF EXISTS (SELECT * FROM [sys].[identity_columns] WHERE [name] IN (N'Id', N'Name') AND [object_id] = OBJECT_ID(N'[Lookup].[FeeTypes]'))
        SET IDENTITY_INSERT [Lookup].[FeeTypes] OFF;
END;
GO
...
BEGIN
    EXEC(N'UPDATE [Lookup].[RegistrationFees] SET [EffectiveTo] = ''9999-08-31T23:59:59.0000000Z''
    WHERE [Id] = 1;
    SELECT @@ROWCOUNT');
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20251120174915_extend-effectiveto-dates', N'8.0.4');
END;
GO
