...
{
    public class SubsidiaryLateFeeCalculationStrategy : ISubsidiaryLateFeeCalculationStrategy<ProducerRegistrationFeesRequestDto, decimal>
    {
        private readonly IProducerFeesRepository _feesRepository;

        public SubsidiaryLateFeeCalculationStrategy(IProducerFeesRepository feesRepository)
        {
        ...
        {
            if (request.NumberOfLateSubsidiaries <= 0)
            {
                return 0m;
            }
            ...
            return await _feesRepository.GetSubsidiaryLateFeeAsync(regulator, request.SubmissionDate, cancellationToken);
        }
    }
}
